# React(Next.js) 프로젝트 - 6단계 Harness Engineering

이 프로젝트는 React(Next.js) 코드 컨벤션을 6단계 서브에이전트 파이프라인으로
...
이 프로젝트는 특정 패키지 매니저를 강제하지 않는다.
프로젝트에 존재하는 락파일을 기준으로 적절한 패키지 매니저를 사용한다.
...
락파일이 없으면 `npm`을 기본으로 사용하되, 사용자에게 확인한다.
**규칙**: 하나의 프로젝트에 락파일 2개 이상 혼용 금지.
...
npx jest               # Jest 단위/통합 테스트
npx jest --watch       # Jest watch 모드
npx jest --coverage    # 커버리지 리포트
npx playwright test    # Playwright E2E 테스트
npm run dev            # Next.js 개발 서버
npm run build          # Next.js 프로덕션 빌드
npm run start          # Next.js 프로덕션 서버
```

> 위 명령은 `npx`/`npm run` 기반 예시이다. 프로젝트의 패키지 매니저에 맞게
> `yarn`/`pnpm`으로 대체 가능.
...
`.claude/skills/react-conventions/` 폴더의 스킬 문서가 단일 진실 소스(SST).
모든 코드 결정은 해당 문서의 섹션 번호를 인용해야 한다.

핵심 금지 사항 (gotchas.md 전체 참조):
- `window.alert()` / `window.confirm()` 사용 → Zustand 전역 모달 또는 Toast 사용
- `any` 타입 → `unknown` + 타입가드
- 인라인 스타일 (`style={{}}`) → styled-components 또는 Tailwind className
- 색상/폰트 하드코딩 → `colors`, `typography` 객체 또는 Tailwind 커스텀 토큰
- 비즈니스 로직을 컴포넌트/훅에 작성 → `services/`로 이동
- `'use client'` 남용 → Server Component 우선
- styled-components + Tailwind 혼용 → 프로젝트당 하나만 선택
...
2. 복잡한 작업이면 곧바로 코드를 작성하지 말고 `/react-build` 안내.
3. React 컨벤션 위반을 발견하면 반드시 `gotchas.md`의 항목 번호 인용.
4. Server Component를 우선 고려하고, 필요한 경우에만 `'use client'` 추가.
