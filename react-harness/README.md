# React Harness Engineering

Next.js + React + TypeScript 프로젝트의 코드 컨벤션을 6단계 서브에이전트
...
- **Claude Code CLI** 2.1.111 이상
- **Node.js** 18 이상
- **패키지 매니저**: yarn, pnpm, npm 모두 지원 (프로젝트에 맞게 사용)
- **GitHub Personal Access Token** (PAT) — repo + read:org 스코프
- 본 하네스가 가정하는 React 컨벤션 스킬 파일들이 이미
  `.claude/skills/react-conventions/` 폴더에 배치되어 있어야 함
...
이 압축파일을 프로젝트 루트에서 풀고, React 컨벤션 스킬 파일을
`.claude/skills/react-conventions/` 폴더에 배치한다.
...
```
> /help
# /react-build, /react-plan, /react-review 표시 확인
```
...
```
> /react-build "사용자가 상품을 즐겨찾기에 추가/제거할 수 있는 기능"
```
...
|---|---|---|---|---|
| ① | `@architect` | 건축가 | Opus (plan 모드) | 첫 결정의 무게, 추론 깊이 |
| ② | `@auditor` | 감사관 | Sonnet | 적대적 페르소나 + 모델 등급 차이로 self-bias 회피 |
| ③ | `@craftsman` | 장인 | Sonnet | 어려운 결정 끝난 후의 구현 — 비용/품질 균형 |
| ④ | `@diagnostician` | 내과의 | Haiku | 체크리스트 대조 — 빠르고 저렴해야 |
| ⑤ | `@reporter` | 기자 | Sonnet | 글쓰기 품질 |
| ⑥ | `@mentor` | 스승 | Opus (effort: high) | 최종 게이트, self-verification |

**비용**: 풀 파이프라인 1회 약 **$3~$8** (변경 규모에 따라 다름).
단순 작업은 `/react-build`를 쓰지 말고 직접 처리하라.
...
| Hook | 이벤트 | 차단 대상 |
|---|---|---|
| `block-forbidden-patterns.sh` | Edit/Write 직전 | `window.alert(`, `window.confirm(`, `: any`, 인라인 스타일, 색상 hex 하드코딩, Tailwind 색상 하드코딩 |
| `autofix-after-edit.sh` | Edit/Write 직후 | ESLint `--fix`로 자동 정리 (패키지 매니저 자동 감지) |
