...
#
# 차단 대상:
#   1. window.alert( / window.confirm(  — gotchas#1
#   2. : any 타입                       — gotchas#2
#   3. style={{                         — gotchas#3 (인라인 스타일)
#   4. 색상 하드코딩                     — gotchas#4
#   5. Tailwind 색상 하드코딩            — gotchas#4
#
# 단, 다음 파일은 예외:
...
# 1. window.alert / window.confirm (gotchas#1)
if echo "$NEW_CONTENT" | grep -qE 'window\.alert\(|window\.confirm\('; then
  VIOLATIONS+=("🔴 gotchas#1: window.alert()/window.confirm() 사용 금지. Zustand 전역 모달 또는 Toast 사용.")
fi
...
case "$FILE_PATH" in
  *.tsx|*.jsx)
    if echo "$NEW_CONTENT" | grep -qE 'style=\{\{'; then
      VIOLATIONS+=("🔴 gotchas#3: 인라인 스타일 금지. styled-components 또는 Tailwind className 사용.")
    fi
    ;;
esac

# 4. 색상 하드코딩 (gotchas#4) — 컴포넌트/훅 파일만, colors/typography 정의 파일 제외
case "$FILE_PATH" in
  *"/styles/colors."*|*"/styles/typography."*|*"tailwind.config"*) ;;
  *.tsx|*.ts)
    # CSS 컬러 hex 패턴 (3, 6, 8자리 hex)
    if echo "$NEW_CONTENT" | grep -qE "'#[0-9a-fA-F]{3,8}'|\"#[0-9a-fA-F]{3,8}\""; then
      VIOLATIONS+=("🔴 gotchas#4: 색상 하드코딩 금지. colors 객체 또는 Tailwind 커스텀 토큰 사용.")
    fi
    ;;
esac

# 5. Tailwind 색상 하드코딩 (gotchas#4) — text-[#xxx], bg-[#xxx], border-[#xxx]
case "$FILE_PATH" in
  *"/styles/colors."*|*"/styles/typography."*|*"tailwind.config"*) ;;
  *.tsx|*.jsx)
    if echo "$NEW_CONTENT" | grep -qE 'text-\[#|bg-\[#|border-\[#'; then
      VIOLATIONS+=("🔴 gotchas#4: Tailwind 색상 하드코딩 금지. Tailwind 커스텀 토큰(text-primary 등) 사용.")
    fi
    ;;
esac
