...
### 3) SSR/SSG 적절성 (Next.js 특화)
- Server Component / Client Component 경계가 적절한가?
- `'use client'`가 트리 하단에 배치되었는가?
- Server Component에서 훅 사용하지 않았는가?
- 데이터 패칭이 서버에서 가능한데 클라이언트에서 하고 있지 않은가?
- `error.tsx`, `loading.tsx`가 적절히 배치되었는가?

### 4) 아키텍처 결정 (architect 결정 재고)
- 레이어 책임 범위 침범 없는가
...
(Server/Client Component 경계 적절성, 서버 렌더링 최대화 여부)
...
- [ ] Server/Client Component 경계를 검증했는가?
