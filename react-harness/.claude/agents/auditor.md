...
- [ ] 컴포넌트 파일명 PascalCase.tsx, Next.js 특수 파일 소문자, 나머지 camelCase.ts?
...
### 컴포넌트 (components.md)
- [ ] `'use client'`가 필요한 컴포넌트에만 적용? 남용 흔적 → BLOCKER
- [ ] Server Component에서 훅(useState, useEffect) 사용 계획 → 즉시 BLOCKER
- [ ] 인라인 `style` 사용 계획 있음 → 즉시 BLOCKER
- [ ] 색상/폰트 하드코딩 계획 있음 → 즉시 BLOCKER
- [ ] styled-components / Tailwind 혼용 흔적 → 즉시 BLOCKER
...
- [ ] `window.alert()` / `window.confirm()` 사용 계획 → 즉시 BLOCKER
- [ ] 기술적 에러 메시지(`error.message`) 노출 → 즉시 BLOCKER
- [ ] Toast vs 모달 분류가 명확?

### 라우팅 (routing.md)
- [ ] 신규 페이지의 Server/Client 결정이 적절?
- [ ] 동적 라우트 params 타입 정의?
- [ ] error.tsx / loading.tsx 필요 여부 명시?
