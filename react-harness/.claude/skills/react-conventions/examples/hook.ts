 ...
 * - [9.3] 반환값 타입을 interface로 명시
 * - [10.3] useCallback: useEffect 의존성 포함 함수에만 사용
...
// [9.3] 반환값 타입은 interface로 명시 — [5.2] 클라이언트 전용 → interface
...
const useUserProfile = (): UseUserProfileReturn => {
  ...
  // [10.3] 캐시 무효화 함수 — useEffect 의존성 포함 가능성 → useCallback
  const invalidate = useCallback(() => {
    queryClient.invalidateQueries({queryKey: userProfileQueryKey});
  }, [queryClient]);

  // [9.3] 일관된 반환 구조: 데이터, 로딩, 에러, 액션 함수
  return {
    profile,
    isFetching,
